...
get_radolan_urls <- function() {
  ...
  base_urls <- kwb.utils::resolveAll(base_url_list)
  ...
  daily_year <- format(seq(lubridate::ymd(daily_date_start),
                           lubridate::ymd(daily_date_end),
                           by = 'months'),
                       format = "%Y")

  daily_month <- format(seq(lubridate::ymd(daily_date_start),
                            lubridate::ymd(daily_date_end),
                            by = 'months'),
                        format = "%m")
  ...
  hourly_year <- format(seq(lubridate::ymd(hourly_date_start),
                            lubridate::ymd(hourly_date_end),
                            by = 'months'),
                        format = "%Y")

  hourly_month <- format(seq(lubridate::ymd(hourly_date_start),
                             lubridate::ymd(hourly_date_end),
                             by = 'months'),
                         format = "%m")
  ...
  url_daily_hist_file <- function() {
    tmp_daily_hist <- sprintf("%s/%s/SF-%s%s.tar.gz",
  ...
  url_hourly_hist_file <- function() {
    tmp_hourly_hist <- sprintf("%s/%s/RW-%s%s.tar.gz",
  ...
  list(daily_historical_urls = url_daily_hist_file(),
...
download_radolan <- function(temporal_resolution = "daily",
                             export_dir = "data") {
  ...
  if (temporal_resolution == "daily") {
    kwb.utils::catAndRun(messageText = "Download: 'daily' historical radolan data",
                         expr = {
                             ...
                             kwb.utils::catAndRun(messageText = msg,
                                                  expr = {
                                                    try(download.file(url = url,
                                                                      destfile = export_path,
                                                                      mode = "wb"))})
                           })})
  }

  if (temporal_resolution == "hourly") {
    kwb.utils::catAndRun(messageText = "Download: 'hourly' historical radolan data",
                         expr = {
                           sapply(urls$hourly_historical_urls, FUN = function(url) {
                             hourly_hist_dir <- sprintf("%s/hourly/historical",
                                                        export_dir)
                             fs::dir_create(hourly_hist_dir, recursive = TRUE)
                             export_path <- sprintf("%s/%s",
                                                    daily_hist_dir,
                                                    basename(url))
                             msg <- sprintf('Download: "daily, historical" and save to %s',
                                            export_path)
